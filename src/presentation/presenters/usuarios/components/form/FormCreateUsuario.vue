<template>
    <q-form @submit.prevent.stop="() => submit()" greedy>
        <div class="flex gap-4">
            <q-input outlined class="basis-[700px] flex-1" dense v-model="form.nome" lazy-rules label="Nome completo"
                clearable clear-icon="close" :rules="nameRules" />

            <q-input outlined class="basis-[500px] flex-1" dense v-model="form.email" lazy-rules label="Email" clearable
                clear-icon="close" :rules="emailRules" />

            <q-select outlined class="basis-[200px] flex-1" dense v-model="form.situacao" :options="situacaoOpt"
                lazy-rules label="Situação de atividade" clear-icon="close" :rules="campoVazioRules" />

            <q-input outlined class="basis-[400px] flex-1" dense v-model="form.login" lazy-rules label="Nome de login"
                clearable clear-icon="close" :rules="loginRules" />

            <q-input outlined class="basis-[400px] flex-1" type="password" dense v-model="form.senha" lazy-rules
                label="Senha de acesso" clear-icon="close" :rules="senhaRules" />

            <q-input outlined class="basis-[400px] flex-1" dense v-model="confirmacao_senha" lazy-rules
        ...
        <div class="flex items-center justify-end gap-2 pt-6">
            ...
            <q-btn @click="$router.back()" v-close-popup label="Cancelar" class="cti-button" />
        </div>
    </q-form>
</template>
